...
const cartSlice=createSlice({
    ...
    reducers:{
        ...
        increaseQuantity:(state,action:PayloadAction<mongoose.Types.ObjectId>)=>{
            const item=state.carData.find(i=>i._id==action.payload)
            if(item){
        ...
        decreaseQuantity:(state,action:PayloadAction<mongoose.Types.ObjectId>)=>{
            ...
            if(item?.quantity && item.quantity>1){
                ...
                state.carData = state.carData.filter(i=>i._id!==action.payload)
            }
            cartSlice.caseReducers.calculateTotal(state)
        },
        ...
        calculateTotal:(state)=>{
            state.subTotal=state.carData.reduce((sum,item)=>sum +Number(item.price)*item.quantity,0)
